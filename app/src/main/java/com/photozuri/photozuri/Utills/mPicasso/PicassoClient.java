package com.photozuri.photozuri.Utills.mPicasso;
...
public class PicassoClient {
    ...
    public static void LoadImage(final Context c, String url, ImageView img, final TextView textView) {
        ...
        if (url != null && url.length() > 0) {
            try {
                Picasso.with(c).load(url).placeholder(R.drawable.image_placeholder).into(img);
                ...
                Picasso.with(c).load(R.drawable.image_placeholder).into(img);
            }
    ...
    public static void LoadImage(final Context c, int re, ImageView img) {


        // if (url != null && url.length() > 0) {
        try {
            Picasso.with(c).load(re).placeholder(R.drawable.image_placeholder).into(img);
